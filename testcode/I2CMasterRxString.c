...
int main(){
	...
	while(1){
		...
		if((buttonRead = GPIO_readFrmInputPin(GPIOA,PIN6))==FALSE){
			...
			I2C_MasterTx(&I2C1Master,&I2CTxMsg);

			I2C_MasterRx(&I2C1Master,&I2CRxMsg);
			printf("data length is %d\n\r",I2CRxMsg.msgBuffer[0]);

			//store the no. of bytes to be read from slave in the Rx mesg length field
			I2CRxMsg.msgLen = I2CRxMsg.msgBuffer[0];
			I2CRxMsg.msgBuffer = &I2CRxMsg.msgBuffer[1];

			//send the cmd to get data
			bufferTx = READ_DATA;
			I2C_MasterTx(&I2C1Master,&I2CTxMsg);

			I2C1Master.I2CConfig.I2C_RepeatedStart = DISABLE;
			I2C_MasterRx(&I2C1Master,&I2CRxMsg);
